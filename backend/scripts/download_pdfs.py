...
def download_pdfs(limit=None, where="status_download != 'sucesso'"):
    ...
    # Usa a condição personalizada passada por argumento
    query = f"SELECT id, url, arquivo FROM pdfs WHERE {where}"
    if limit:
        query += f" LIMIT {limit}"
    ...
    if not pending_pdfs:
        print(f"Nenhum PDF encontrado com a condição: {where}")
        conn.close()
        return

    print(f"Iniciando tentativa para {len(pending_pdfs)} PDFs (Condição: {where}).")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Downloader de PDFs da ANEEL")
