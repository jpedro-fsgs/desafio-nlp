...
def get_agent_tools() -> list:
    """Inicializa as ferramentas com descrições estratégicas para o Agente."""

    # 1. Ferramenta de busca macro (Registros)
    async def pesquisar_registros(
        query: str, registro_id: Optional[int] = None, situacao: Optional[str] = None
    ) -> ToolResponseModel:
        """
        ...
        """
        logger.info(
            f"[TOOL:pesquisar_registros] Query: '{query}' | ID: {registro_id} | Situação: {situacao}"
        )
        try:
            filter_list = []
            ...
            if registro_id:
                filter_list.append(
                    ExactMatchFilter(key="registro_id", value=registro_id)
                )
            if situacao:
            ...
            if filter_list:
                logger.debug(
                    f"[TOOL:pesquisar_registros] Filtros aplicados: {[f.key for f in filter_list]}"
                )
            ...
            if hasattr(response, "source_nodes"):
                ...
                logger.info(
                    f"[TOOL:pesquisar_registros] Encontrados {len(response.source_nodes)} nós de origem."
                )
            ...
            return ToolResponseModel(
                text=f"Erro técnico na busca de registros: {e}", sources=[]
            )
    ...
    async def pesquisar_documentos_tecnicos(
        ...
        """
        logger.info(
            f"[TOOL:pesquisar_documentos] Query: '{query}' | RegID: {registro_id} | PDF: {pdf_nome} | Natureza: {natureza}"
        )
        try:
            filter_list = []
            if registro_id:
            ...
            if filter_list:
                logger.debug(
                    f"[TOOL:pesquisar_documentos] Filtros aplicados: {[f.key for f in filter_list]}"
                )
            ...
            logger.debug(
                f"[TOOL:pesquisar_documentos] Executando query técnica no engine..."
            )
            response = await engine.aquery(query)

            if hasattr(response, "source_nodes"):
                logger.info(
                    f"[TOOL:pesquisar_documentos] Encontrados {len(response.source_nodes)} trechos técnicos."
                )
            ...
            logger.error(
                f"[TOOL:pesquisar_documentos] Erro técnico profundo: {e}", exc_info=True
            )
            return ToolResponseModel(
                text=f"Erro técnico na busca profunda: {e}", sources=[]
            )
    ...
    async def ler_documento_completo(pdf_nome: str) -> ToolResponseModel:
        ...
        try:
            ...
            if content:
                logger.info(
                    f"[TOOL:ler_documento] Sucesso ao carregar {pdf_nome} ({len(content)} caracteres)."
                )
                source = SourceModel(
                    id=pdf_nome,
                    title=pdf_nome,
            ...
            return ToolResponseModel(
                text=f"Aviso: O arquivo {pdf_nome} não foi encontrado para leitura integral.",
                sources=[],
            )
        except Exception as e:
            logger.error(
                f"[TOOL:ler_documento] Erro ao ler documento integral: {e}",
                exc_info=True,
            )
            return ToolResponseModel(
                text=f"Erro técnico ao acessar o conteúdo integral: {e}", sources=[]
            )
    ...
    return [
        FunctionTool.from_defaults(
            ...
            description=(
                "Localiza normas regulatórias (Resoluções, Portarias, Despachos) por tema ou ID, "
                "retornando metadados como registro_id, título, ementa, data e situação de vigência.\n"
                "A Ementa fornece um resumo conciso da norma, útil para entender rapidamente seu conteúdo.\n"
                "FILTROS DISPONÍVEIS:\n"
                "  - query (obrigatório): busca semântica na ementa da norma.\n"
                "  - registro_id: use quando o usuário mencionar um ID específico — garante busca direta e precisa.\n"
                "  - situacao: filtra pelo estado da norma. Valores exatos aceitos:\n"
                "      'NÃO CONSTA REVOGAÇÃO EXPRESSA' → normas vigentes,\n"
                "      'REVOGADA', 'SUSPENSA', 'TORNADA SEM EFEITO'.\n"
                "      Omita o filtro quando quiser resultados mistos ou ao rastrear a cadeia de revogação — "
                "normas REVOGADAS contêm nos metadados a referência à norma que as substituiu.\n"
                "QUANDO USAR: sempre que precisar identificar quais normas existem sobre um tema "
                "ou obter o registro_id para aprofundar a busca com outras ferramentas."
            ),
        ...
        FunctionTool.from_defaults(
            ...
            description=(
                "Busca vetorial no conteúdo de documentos vinculados a normas e registros: "
                "Votos, Notas Técnicas, Anexos, Despachos e Resoluções Autorizativas.\n"
                "ATENÇÃO: retorna apenas chunks (fragmentos) dos documentos mais relevantes — "
                "NÃO o documento completo. Se o trecho encontrado não for suficiente para responder "
                "com precisão, use imediatamente 'ler_documento_completo_direto' com o pdf_nome retornado.\n"
                "FILTROS DISPONÍVEIS:\n"
                "  - query (obrigatório): termos técnicos ou pergunta específica sobre o conteúdo do documento.\n"
                "  - registro_id: restringe a busca aos documentos de uma norma específica — "
                "use sempre que já tiver o ID para evitar resultados de outras normas.\n"
                "  - pdf_nome: use quando já souber o arquivo exato (ex: 'ren20211000.pdf').\n"
                "  - natureza: filtra pelo tipo do documento. Exemplos:\n"
                "      'Voto' → justificativa e motivação da decisão do relator,\n"
                "      'Nota Técnica' → análise técnica da área responsável,\n"
                "      'Anexo' → tabelas, metodologias e fórmulas detalhadas,\n"
                "      'Despacho', 'Resolução Autorizativa'.\n"
                "      Para entender POR QUE uma decisão foi tomada, prefira natureza='Voto'.\n"
                "  - sigla: filtra pelo tipo do ato regulatório (ex: REN, REA, REH, DSP).\n"
                "QUANDO USAR: como ponto de entrada para localizar em qual documento e trecho "
                "está a informação buscada. Para análise completa, sempre complemente com "
                "'ler_documento_completo_direto'."
            ),
        ...
        FunctionTool.from_defaults(
            ...
            description=(
                "Lê o conteúdo Markdown integral de um PDF específico.\n"
                "PARÂMETRO:\n"
                "  - pdf_nome (obrigatório): nome exato do arquivo (ex: 'rea202212715ti.pdf'). "
                "Obtido nos metadados retornados pelas outras ferramentas.\n"
                "QUANDO USAR:\n"
                "  - Sempre que 'pesquisar_documentos_pdf_aneel' retornar chunks relevantes mas insuficientes "
                "— leia o documento completo imediatamente para garantir contexto e precisão.\n"
                "  - Para analisar a estrutura completa do documento (artigos, anexos, tabelas, ordem).\n"
                "  - Para leitura direta quando o pdf_nome já for conhecido, sem necessidade de busca prévia.\n"
                "Use sem hesitação sempre que o documento completo agregar precisão à resposta."
            ),
